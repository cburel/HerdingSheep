...
player = Dog(dogImage, Vector(Constants.PLAYER_XPOS, Constants.PLAYER_YPOS), (Constants.PLAYER_SIZE), Constants.PLAYER_SPD, Constants.PLAYER_COLOR, Constants.PLAYER_TURN_SPEED)
bounds = Vector(Constants.DISPLAY_WIDTH, Constants.DISPLAY_HEIGHT)

enemies = []
for i in range (1, Constants.MAX_ENEMIES + 1):
	enemies.append(Sheep(sheepImage, Vector(random.randrange(0, bounds.x), random.randrange(0, bounds.y)), Constants.ENEMY_SIZE, Constants.ENEMY_SPD, Constants.ENEMY_COLOR, Constants.ENEMY_TURN_SPEED))
...
while True:
	...
	for enemy in enemies:
		enemy.update(bounds, screen, player, enemies)
